...
protectcodes()
{
    //The basic implementation is to generate a sysreq.n command and pass an invalid 
    //native function number to force an error on the decompiler. And we use jump
    //opcode to never execute the error native call.
    int label = getlabel();
    jumplabel(label);

    stgwrite("\tsysreq.n ");

    stgwrite("__Yourmnbbn__Anti__Decompiler__");
    stgwrite(" ");
    outval(11, FALSE);
    stgwrite("\n");
    code_idx += opcodes(1) + opargs(2);
