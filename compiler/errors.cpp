...
static unsigned char warnerror[(NUM_WARNINGS + 7) / 8]; /* 8 flags in a char */
...
ErrorReport::create_va(int number, int fileno, int lineno, va_list ap)
{
    ...
    if (report.type == ErrorType::Warning) {
        ...
        else if((warnerror[index] & mask) != 0)
            report.type = ErrorType::Error;
    }
...
int
pc_enablewarningerror(int number, int enable)
{
    int index;
    unsigned char mask;
    ...
    switch (enable) {
        case 1:
            warnerror[index] |= mask;
            break;
        case 0:
            warnerror[index] &= (unsigned char)~mask;
            break;
        case 2:
            warnerror[index] ^= mask;
            break;
