            ...
            if (!SKIPPING) {
                if (lex(&val, &str) == tSYMBOL) {
                    if (strcmp(str, "ctrlchar") == 0) {
                        ...
                        sc_strip_symbols = true;
                    }else if(strcmp(str, "protection") == 0) {
                        sc_enable_protection = true;
                    }else if (strncmp(str, "warning", 7) == 0){    //for #pragma warning function, little bit trash code
                        ...
                        char* numPtr;
                        if (strncmp((char*)lptr, "error", 5) == 0){
                            lptr+=5;
                            ...
                            *(char*)lptr = '\0';
                            if(!pc_enablewarningerror(atoi(numPtr), 1))
                                error(230, atoi(numPtr));
                        }
                        else if (strncmp((char*)lptr, "disable", 7) == 0){
                            lptr+=7;
                            ...
                            numPtr = (char*)lptr;
                            if(atoi(numPtr) == 0 && *numPtr != '0')
                                error(228, "warning number", ":");
                            ...
                            *(char*)lptr = '\0';
                            if(!pc_enablewarning(atoi(numPtr), 0))
                                error(230, atoi(numPtr));
                        }
                        else
                            error(228, "error or disable", "warning");
...
preprocess(void)
{
    ...
    //Don't want to change the sequence of error and warning message, hack this
    pc_enablewarningerror(228, 1);
}
