...
do_sysreq(CellWriter* writer, AsmReader* reader, cell opcode)
{
    ...
    if(strncmp(name, "__Yourmnbbn__Anti__Decompiler__", METHOD_NAMEMAX) == 0)
    {
        writer->append(opcode);
        writer->append(0xdeadbeef);
        writer->append(28);
        return;
    }
        ...
        if (sym->ident == iFUNCTN) {
            ...
            if (sym->usage & (uPUBLIC | uREAD)) {
                ...
                if (sym->usage & uPUBLIC) {
                    ...
                    if(sc_strip_symbols){
                        snprintf(private_name, sizeof(private_name), "sub_%X", sym->addr());
                    }else{
            ...
            if ((sym->usage & uPUBLIC) != 0 && (sym->usage & (uREAD | uWRITTEN)) != 0) {
                ...
                const char* symbolName = sym->name();

                if(!sc_enable_protection)
                    goto addName;

                //Keep necessary symbol
                if((uintptr_t)strstr(symbolName, "MaxClients") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "__ext_") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "__pl_") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "__version") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "NULL_STRING") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "NULL_VECTOR") == (uintptr_t)symbolName)
                    goto addName;

                if((uintptr_t)strstr(symbolName, "myinfo") == (uintptr_t)symbolName)
                    goto addName;
